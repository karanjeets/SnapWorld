	...
	public String getData(String latitude, String longitude)
	{
		Connection c = null;
	      ...
	      try {
	        ...
	        ResultSet rs = stmt.executeQuery("SELECT T.*, C.name as category_name FROM (SELECT *, (SELECT SUM(di.cost)*69 FROM pgr_dijkstra('SELECT id, source, " +
                    ...
                    "AS T INNER JOIN category C ON T.category = C.id ORDER BY T.road_distance;" );
